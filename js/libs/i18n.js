(function(window) {
  ...
  findTemplate = function(key, setOnMissing) {
    //assert("You must provide a translation key string, not %@".fmt(key), typeof key === 'string');
	  assert(Ember.String.fmt("You must provide a translation key string, not %@", [key]), typeof key === 'string');
  ...
  I18n = {
    ...
    template: function(key, count) {
      ...
      if ((count != null) && (pluralForm != null)) {
        ...
        interpolatedKey = Ember.String.fmt("%@.%@", [key, suffix]);
  ...
  Handlebars.registerHelper('t', function(key, options) {
    ...
    var _attr_ = Ember.A(Em.keys(attrs));
    _attr_.forEach(function(property) {
    ...
    result = Ember.String.fmt('<%@ id="%@">%@</%@>', [tagName, elementID, I18n.t(key, attrs), tagName]);
  ...
  Handlebars.registerHelper('translateAttr', function(options) {
    ...
    Em.keys(attrs).forEach(function(property) {
      ...
      return result.push(Ember.String.fmt('%@="%@"', [property, translatedValue]));
    });
